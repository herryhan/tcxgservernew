...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component{
    return  [(NSArray *)self.dataArray[component] count] * 200;
}
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    ...
    switch (component) {
        case 0: { // 年
            ...
            NSString *year_integerValue = self.yearArr[row%[(NSArray*)self.dataArray[component] count]];
        ...
        case 1: { // 月
            ...
            NSString *month_value = self.monthArr[row%[(NSArray *)self.dataArray[component] count]];
        ...
        case 2: { // 日
            ...
            NSString *dayStr = [self getDayNumber:[self.year integerValue] month:[self.month integerValue]];
            // 如果选择年大于当前年 就直接赋值日
            NSString *day_value = self.dayArr[row%[(NSArray *)self.dataArray[component] count]];
            NSLog(@"%ld", (unsigned long)self.dayArr.count);
            if ([self.year integerValue] > [self.timeArr[0] integerValue]) {
                if (self.dayArr.count <= [dayStr integerValue]) {
                    self.day = day_value;
                } else {
                ...
                if ([self.month integerValue] > [self.timeArr[1] integerValue]) {
                    ...
                    if (day_value.integerValue < [self.timeArr[component] integerValue]) {
                        ...
                        if (self.dayArr.count <= [dayStr integerValue]) {
                            self.day = self.dayArr[row%[(NSArray *)self.dataArray[component] count]];
                        } else {
                            if ([self.dayArr[row%[(NSArray *)self.dataArray[component] count]] integerValue] <= [dayStr integerValue]) {
                                self.day = self.dayArr[row%[(NSArray *)self.dataArray[component] count]];
                            } else {
                                [pickerView selectRow:[self.dataArray[component] indexOfObject:[dayStr stringByAppendingString:@"日"]] inComponent:component animated:YES];
                            }
        ...
        case 3: { // 时
            // 如果选择年大于当前年 就直接赋值时
            if ([self.year integerValue] > [self.timeArr[0] integerValue]) {
                self.hour = self.hourArr[row%[(NSArray *)self.dataArray[component] count]];
                // 如果选择的年等于当前年，就判断月份
            } else if ([self.year integerValue] == [self.timeArr[0] integerValue]) {
                ...
                if ([self.month integerValue] > [self.timeArr[1] integerValue]) {
                    ...
                    if ([self.day integerValue] > [self.timeArr[2] integerValue]) {
                        ...
                        // 如果选择的时小于当前时，就刷新到当前时
                        if ([self.hourArr[row%[(NSArray *)self.dataArray[component] count]] integerValue] < [self.timeArr[3] integerValue]) {
                            [pickerView selectRow:[self.dataArray[component] indexOfObject:self.timeArr[component]] inComponent:component animated:YES];
                            // 如果选择的时大于当前时，就直接赋值
                        } else {
                            self.hour = self.hourArr[row%[(NSArray *)self.dataArray[component] count]];
        ...
        case 4: { // 分
            // 如果选择年大于当前年 就直接赋值时
            //            if ([self.year integerValue] > [self.timeArr[0] integerValue]) {
            self.minute = self.minuteArr[row%[(NSArray *)self.dataArray[component] count]];
            //                // 如果选择的年等于当前年，就判断月份
            //            } else if ([self.year integerValue] == [self.timeArr[0] integerValue]) {
            //                // 如果选择的月份大于当前月份 就直接复制时
            //                if ([self.month integerValue] > [self.timeArr[1] integerValue]) {
...
- (nullable NSString *)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component{
    return  [self.dataArray[component] objectAtIndex:row%[(NSArray *)self.dataArray[component] count]];
}
...
-(UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view {
    ...
    titleLbl.text = [self.dataArray[component] objectAtIndex:row%[(NSArray *)self.dataArray[component] count]];
